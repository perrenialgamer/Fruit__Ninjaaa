...
//hereeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee
for (let i = 0; i < maxTrailElements; i++) {
    const trailElement = document.createElement('div');
    trailElement.classList.add('circle-trail');
    trailElement.style.display = 'none'; // Hide initially
...
function createCircleTrail(x, y) {
    ...
    setTimeout(() => {
        trailElement.style.opacity = '0'; // Fade out effect
    }, 100);

    setTimeout(() => {
        trailElement.style.display = 'none'; // Hide after fade out
    }, 400); // Match the duration with the opacity transition

    currentTrailIndex = (currentTrailIndex + 1) % maxTrailElements;
}

function updateCustomCursor(x, y) {
    // Calculate the position relative to the game canvas
    const gameAreaRect = gameArea.getBoundingClientRect();
    const offsetX = x - gameAreaRect.left;
    const offsetY = y - gameAreaRect.top;
...
document.addEventListener('DOMContentLoaded', () => {
    customCursor.style.display = 'block'; // Show the custom cursor by default
